...
class AnalysePresenterTests {
    ...
    private lateinit var roleUtils: Role
    ...
    @Before
    fun setUpPresenter() {
        roleUtils = mock(Role::class.java)
...
//        arrayList.add(mock(AnalysisStatUrls::class.java))
        val champ = OldChamp()
        champ.champName = "Vi"
        `when`(roleUtils.produceSingleLaneStringForRoleInt(4)).thenReturn("SUPPORT")
...
//        val statList = StatList(arrayList, 0)
    ...
    fun ensureThatIllegalStateExceptionGetsThrownIfWeTryToCreateAMessageWithoutARoleBeingSet() {
        ...
        val champ = OldChamp()
        champ.champName = "Vi"
        `when`(roleUtils.produceSingleLaneStringForRoleInt(6)).thenThrow(IllegalStateException())
        analysePresenter.setChamp(champ)
    ...
    fun ensureWeOnlyRequestRoleFilterIfWeHaveNoChampSet() {
        analysePresenter.setRole(4)
        analysePresenter.start()
        verify<AnalyseInteractor>(interactor, times(1)).loadStatList("ADC", analysePresenter, "OCE", "-1")
    }

    @Test
    fun ensureThatWeFetchForChampAndRoleIfWeHaveAChampFilterSet() {
        analysePresenter.setRole(1)
        val champ = mock(OldChamp::class.java)
        `when`(champ.champId).thenReturn(31)
        analysePresenter.setChamp(champ)
        analysePresenter.start()
        verify<AnalyseInteractor>(interactor, times(1)).loadStatList("TOP", 31, analysePresenter, "OCE")
    }
